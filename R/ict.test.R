ict.test <- function(y, treat, J, alpha = 0.05, n.draws = 250000, gms = FALSE){
  ...
  y.comb.all <- c(0:(J-1), 1:J)
  t.comb.all <- c(rep(1, J), rep(0, J))
  ...
  if (sum(cond.y1 == FALSE) + sum(cond.y0 == FALSE) > 0){

    y.comb <- t.comb <- NA
    for(i in 1:J){
      if (cond.y1[i] == FALSE) {
        y.comb <- c(y.comb, (0:(J-1))[i])
        t.comb <- c(t.comb, 1)
      }
    }
    for(i in 1:J){
      if (cond.y0[i] == FALSE) {
        y.comb <- c(y.comb, (1:J)[i])
        t.comb <- c(t.comb, 0)
      }
    }
